...
def get_route(origin_x, origin_y, dest_x, dest_y, avoid_place, API_KEY, max_api_calls=50, opennow=True):
    ...
    if len(avoid_place_lst) > 0:
        ...
        while abs((abs(current_node[0])+ abs(current_node[1])) - (abs(destination_x)+ abs(destination_y))) > grid_size and i <= max_api_calls:
            #Test line
